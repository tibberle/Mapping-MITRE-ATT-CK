...
def create_tactic_table():
    tactics_grouped = main_techniques.groupby('tactics')

    # If no techniques found, return a message
    if tactics_grouped.ngroups == 0:
        return html.Div("No techniques found.")

    # Create the header for the table based on tactics
    table_header = [
        html.Tr([html.Th(tactic, style={"background-color": "#f2f2f2", "color": "#003366", "text-align": "center"}) for tactic in tactics_grouped.groups.keys()])
    ]

    table_body = []
    # Determine the maximum number of techniques per tactic for table rows
    max_techniques = max(len(group) for _, group in tactics_grouped) if tactics_grouped.ngroups > 0 else 0

    # Populate the table body with techniques
    for i in range(max_techniques):
        row = []
        for tactic, group in tactics_grouped:
            if i < len(group):
                technique = group.iloc[i]
                # Create a table cell linking to the technique detail page
                row.append(html.Td(
                    html.A(technique['name'], href=f"/techniques/{technique['ID']}", id=f"link-{technique['ID']}", target="_blank", style={"color": "#007BFF", "text-decoration": "none"}),
                    style={"border": "1px solid #ddd", "padding": "10px", "text-align": "center"}
                ))
            else:
                row.append(html.Td('', style={"border": "1px solid #ddd", "padding": "10px"}))
        table_body.append(html.Tr(row))
    
    # Return the complete table structure
    return html.Table(children=table_header + table_body, style={"border-collapse": "collapse", "width": "100%", "margin-top": "20px"})
...
def data_analysis_visualisation():
    ...
    ], style={"padding": "20px", "margin": "20px", "border": "1px solid #ddd", "border-radius": "5px", "background-color": "#f9f9f9"})  # Added margin for consistency
...
def filter_techniques(selected_tactics, selected_platforms):
    ...
    if selected_tactics:
        # Check if all selected tactics are present in the 'tactics' column
    ...
    if selected_platforms:
        # Check if all selected platforms are present in the 'platforms' column
    ...
    return html.Div([count_display] + technique_cards, style={"margin-top": "20px"})  # Fix applied here
...
def render_content(tab, pathname):
    ...
    if tab == 'tab-1':
        ...
        return html.Div([
            ...
            html.Div(id='filter-results', style={"margin-top": "20px"})  # Ensure margin for consistency
